...
- При снижении цены или достижении цели создаётся **уведомление** в приложении
  (страница «Уведомления» с бейджем непрочитанных). Если задать токен бота
  `TELEGRAM_BOT_TOKEN` и указать свой Chat ID в настройках — уведомление приходит и в Telegram.
...
Остальной код (API, очередь, scoring, фронтенд) менять не нужно. Так уже добавлен
**Яндекс.Маркет** (`yandex_market`) — включается через `ENABLED_MARKETPLACES`
(по умолчанию выключен из-за сильной капчи; надёжнее на «чистом» RU-IP, см. `YM_HEADLESS`).

## Что готово (MVP)

- Регистрация/вход (JWT), защищённые маршруты.
- Поиск с выбором маркетплейсов и фильтрами (рейтинг, отзывы, цена), сортировки.
- Реальные данные Wildberries; Ozon и Яндекс.Маркет best-effort через Playwright.
...
- Двусторонний Telegram-бот (подписка/команды), а не только исходящие уведомления.
- DNS, М.Видео и другие маркетплейсы — новыми адаптерами.
