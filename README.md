...
**Яндекс.Маркет** (`yandex_market`) и **DNS** (`dns`) — включаются через `ENABLED_MARKETPLACES`
(по умолчанию выключены из-за сильной анти-бот-защиты; надёжнее на «чистом» RU-IP,
см. `YM_HEADLESS` / `DNS_HEADLESS`).

## Что готово (MVP)

- Регистрация/вход (JWT), защищённые маршруты.
- Поиск с выбором маркетплейсов и фильтрами (рейтинг, отзывы, цена), сортировки.
- Реальные данные Wildberries; Ozon, Яндекс.Маркет и DNS best-effort через Playwright.
...
- М.Видео и другие маркетплейсы — новыми адаптерами.
